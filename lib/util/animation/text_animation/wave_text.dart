  ...
  Widget animatedBuilder(BuildContext context, Widget? child) {
    ...
    return RepaintBoundary(
      child: CustomPaint(
        ...
        child: Text(
          text,
          style: defaultTextStyle.merge(textStyle).merge(const TextStyle(color: Colors.transparent)),
...
class _WTextPainter extends CustomPainter {
  ...
  void calculateMove() {
    ...
    if (txtInMoOdd < (text.length - 1) / 2 && !txtInMoOdd.isNegative) {
      ...
      _textLayoutInfo[txtInMoOdd + (txtInMoOdd + 1)].riseHeight = progress < .5 ? 0 : -1.3 * height * math.sin((progress - .5) * math.pi).abs();
    }
